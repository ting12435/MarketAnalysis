...
void parse() {
	...
	// 	std::ofstream my_file(date_d.first);
	// 	for (const auto &issuer_d: date_d.second) {

	// 		if (issuer_d.first == issuer_name) {
	// 			for (const auto &stock_d: issuer_d.second) {
	// 				my_file << "=\"" << stock_d.first << "\"," << stock_d.second.b_lot << "," << stock_d.second.s_lot << "," << stock_d.second.b_lot - stock_d.second.s_lot << std::endl;
	// 			}
	// 		}
	// 	}
	// 	my_file.close();
	...
	for (const auto &s: m) {
		my_file << "=\"" << s.first << "\"," << s.second.b_lot << "," << s.second.s_lot << "," << s.second.b_lot - s.second.s_lot << std::endl;
	}
	my_file.close();
}
